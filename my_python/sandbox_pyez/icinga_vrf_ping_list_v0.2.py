...
def get_args():
    ...
    parser.add_argument('-l', '--destination_ips', nargs='+', required=True, help='list of IP to ping')
...
remote_host, remote_username, remote_password, vrf, ips, debug = get_args()
...
# python icinga_vrf_ping_list_v0.2.py -H dist2-testlab.nn.hea.net -u heanet -p '$!3u$uxqDMTXzw9' -f scoil_l3vpn.20200507 -l 10.11.12.13 10.11.12.13
print(remote_host, remote_username, remote_password, vrf, ips, debug)
print(remote_host_ipv4, remote_username, remote_password, vrf, ips, debug)
for i in ips:
    print(i)
...
outcome = 0
for ip in ips:
    outcome = dev.rpc.ping({'format': 'json'}, routing_instance=vrf, host=ip, rapid=True)
    results_dict = outcome['ping-results'][0]   # dictionary where we find the success/failure

    if 'ping-success' in results_dict:
        # outcome = "SUCCESS"
        my_outcome = ICINGA_OK
        print(my_outcome)
        outcomes_dict[ip] = "SUCCESS"
    if 'ping-failure' in results_dict:
        # outcome = "FAILURE"
        my_outcome = ICINGA_CRITICAL
        print(my_outcome)
        outcomes_dict[ip] = "FAILURE"
        outcome = 2
