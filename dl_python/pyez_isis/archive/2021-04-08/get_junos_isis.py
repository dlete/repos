...
"""
invoke as:
# all to defaults: all IS-IS interfaces, the default routing-instance,
# and default debug level
outcome = get_junos_isis_interfaces(ne,
                                    os_username,
                                    os_password)

# to change the debug level explicitly
# pass an empty list for the interfaces and empty string for the instance
# isis_interfaces = []
# isis_instance = ''
python get_junos_isis_interfaces(ne,
                                 os_username,
                                 os_password,
                                 isis_interfaces,
                                 isis_instance,
...
python -W ignore check_junos_isis_adjacencies(...)
"""
...
def get_junos_isis_interfaces(ne: str,
    ...
    try:
        ...
        related_information = ('https://pyez.readthedocs.io/en/latest/'
                               'jnpr.junos.html#module-jnpr.junos.exception')
        raise Exception('Error connecting to {ne}: {0}'
                        'Check this, it may help: {related_information}'
                        .format(err, ne=ne, related_information=related_information))
...
def check_isis_consistency(isis_interfaces: dict,
    ...
    '''
    Checkss consistency between the IS-IS router configuration
    ...
    if isis_if_level_consistency is True:
        isis_interfaces['isis_interfaces_consistency'] = True
    else:
        isis_interfaces['isis_interfaces_consistency'] = False
    logger.debug('OVERALL IS-IS interface consistency is: {consistency}'.
                 format(consistency=isis_interfaces['isis_interfaces_consistency']))
...
def example_of_use():
    ...
    # invoke function
    # outcome = get_junos_isis_interfaces(ne, os_username, os_password)
    isis_interfaces = get_junos_isis_interfaces(ne,
                                                os_username,
                                                os_password,
                                                isis_interfaces,
                                                isis_instance,
    ...
    # display the outcome
    print('What follows is the outcome of the script "get_junos_isis_interfaces"')
    pprint(isis_interfaces)

    isis_consistency = check_isis_consistency(isis_interfaces,
                                              debug_level)
    # display the outcome
    print('What follows is the outcome of the script "check_isis_consistency"')
    pprint(isis_consistency)
...
if __name__ == '__main__':
    ...
    Execute when running the module from the command line but
    not when the Python interpreter imports this module.
    """
    example_of_use()
